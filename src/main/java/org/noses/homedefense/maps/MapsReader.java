...
public class MapsReader {
    ...
    public MapDTO readMap(int width,
        ...
        List<WayNode> wayNodes = mapsRepository.getWayNodes(nodes.stream().map(n -> n.getId()).collect(Collectors.toList()));
        ...
        for (Way way : ways) {
            ...
            for (WayNode wayNode : wayNodes) {
                if (wayNode.getWayNodeKey().getWay() == way.getId()) {
                    Node node = nodes.stream().filter(n->n.getId() == wayNode.getWayNodeKey().getNode()).findFirst().get();
